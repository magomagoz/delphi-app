...
def calcola_pronostico_streamlit(nome_input):
    ...
    st.divider()
    st.subheader("🏁 Mercati Classici (U/O & GOL)")
    cuo, cgng = st.columns(2)
    with cuo:
        pu, po = p_u25/total_p, 1-(p_u25/total_p)
        st.info(f"**U2.5:** {pu:.1%} (Q: {stima_quota(pu)})")
        st.info(f"**O2.5:** {po:.1%} (Q: {stima_quota(po)})")
    with cgng:
        pg, png = p_gol/total_p, 1-(p_gol/total_p)
        st.info(f"**GOL:** {pg:.1%} (Q: {stima_quota(pg)})")
        st.info(f"**NOGOL:** {png:.1%} (Q: {stima_quota(png)})")
