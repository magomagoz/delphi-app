...
with tab1:
    ...
    if 'pronostico_corrente' in st.session_state and st.session_state['pronostico_corrente']:
        ...
        cx.metric("X", f"{d['px']:.1%}")
        c2.metric("2", f"{d['p2']:.1%}")
        
        st.write("---")
        st.write(f"**Risultati Esatti:** {d['Top 6 RE Finali']}")
...
with tab2:
    st.info("⚠️ IMPORTANTE: Clicca qui sotto per risolvere l'errore 'KeyError: ID'")
