...
def calcola_pronostico_streamlit(nome_input):
    if not os.path(FILE_DB):
        ...
        if zc_h > 60 or zc_a > 60:
            avviso_late_goal = "⚠️ ATTENZIONE: Alta probabilità di GOL nel finale (80'+)!\n"
            
                    # --- OUTPUT STREAMLIT ---
    st.header(f"{casa} vs {fuori}")
    st.info(f"\n"
            f"🏆 {m['League']} - 📅 {m['Date']}\n"
            f"👮 Arbitro: {arbitro} (Severità: {molt_arbitro})\n"
    ...
    salva_report_permanente(report_text, f"{casa} vs {fuori}")
        ...
        min_p = min(n_h, n_a)
        aff_msg = "🔴 AFFIDABILITÀ DEI DATI: CRITICA" if min_p < 3 else "🟡 AFFIDABILITÀ DEI DATI: MEDIA" if min_p < 5 else "🟢 AFFIDABILITÀ DEI DATI: ALTA"
